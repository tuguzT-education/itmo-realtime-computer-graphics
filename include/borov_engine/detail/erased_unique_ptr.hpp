...
constexpr inline erased_delete default_erased_delete() {
...
constexpr inline erased_delete default_erased_delete() = delete;

using erased_unique_ptr = std::unique_ptr<void, erased_delete>;

template<typename T>
requires (!std::is_array_v<T>)
inline erased_unique_ptr erased_unique(T * ptr) {
    return {ptr, single_erased_delete<T>};
}

template<typename T>
requires std::is_array_v<T>
inline erased_unique_ptr erased_unique(std::remove_extent_t<T> *ptr) {
    return {ptr, array_erased_delete<T>};
}

template<typename T>
requires std::is_bounded_array_v<T>
inline erased_unique_ptr erased_unique(std::remove_extent_t<T> *) = delete;
...
requires (!std::is_array_v<T>)
inline erased_unique_ptr make_erased_unique(Args &&...args) {
    return {new T{std::forward<Args>(args)...}, single_erased_delete<T>};
}

template<typename T>
requires std::is_array_v<T>
inline erased_unique_ptr make_erased_unique(std::size_t size) {
    return {new std::remove_extent_t<T>[size]{}, array_erased_delete<T>};
}

template<typename T, typename... Args>
requires std::is_bounded_array_v<T>
inline erased_unique_ptr make_erased_unique(Args &&...) = delete;

template<typename T>
requires (!std::is_array_v<T>)
inline erased_unique_ptr make_erased_unique_for_overwrite() {
    return {new T, single_erased_delete<T>};
}

template<typename T>
requires std::is_array_v<T>
inline erased_unique_ptr make_erased_unique_for_overwrite(std::size_t size) {
    return {new std::remove_extent_t<T>[size], array_erased_delete<T>};
}

template<typename T, typename... Args>
requires std::is_bounded_array_v<T>
inline erased_unique_ptr make_erased_unique_for_overwrite(Args &&...) = delete;

template<typename T>
requires (!std::is_array_v<T>)
inline erased_unique_ptr erase(std::unique_ptr<T> ptr) {
